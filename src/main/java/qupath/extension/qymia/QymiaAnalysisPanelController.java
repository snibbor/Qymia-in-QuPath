...
public class QymiaAnalysisPanelController extends BaseController implements Initializable {
    ...
    private static final Logger logger = LoggerFactory.getLogger(QymiaAnalysisPanelController.class);
...
//    private ObjectProperty<Future<?>> runningTask = new SimpleObjectProperty<>();
    ...
    private static BooleanProperty flipXYProperty = PathPrefs.createPersistentPreference("flipXYQymiaAnalysis", false);
    @FXML
...
//    File initialFileDirectory;
    ...
    public QymiaAnalysisPanelController(QuPathGUI qupath) {
        this.qupath = qupath;
...
//            lineChart.getData().addAll(scatterSeries, regSeries);
    ...
    void handleTMAStandardCalcRegs() {
        ...
        if (project == null) {
            Dialogs.showNoProjectError("Qymia Analysis");
            return;
        }

        if (!QymiaQuantCommands.checkSaveChangesPrompt(qupath.getImageData(), project)){
//			If the prompt was cancelled by user or it returns false for some reason, do not create and show measurement dialog
        ...
        QymiaAnalysisPanelController.TMAStandardRegTask worker = new QymiaAnalysisPanelController.TMAStandardRegTask(project, imagesToProcess);
