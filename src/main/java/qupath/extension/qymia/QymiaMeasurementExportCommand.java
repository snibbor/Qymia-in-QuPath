...
public class QymiaMeasurementExportCommand implements Runnable {

	private QuPathGUI qupath;
	private static final Logger logger = LoggerFactory.getLogger(QymiaMeasurementExportCommand.class);
	 ...
	 * @param qupath
	 */
	public QymiaMeasurementExportCommand(final QuPathGUI qupath) {
		this.qupath = qupath;
	}
...
//			If the prompt was cancelled by user or it returns false for some reason, do not create and show measurement dialog
		...
		btnChooseFile.setOnAction(e -> {
			...
			File pathOut = Dialogs.promptToSaveFile("Output file", Projects.getBaseDirectory(project), "QymiaScores" + ext, extDesc, ext);
			if (pathOut != null) {
				if (pathOut.isDirectory())
					pathOut = new File(pathOut.getAbsolutePath() + File.separator + "QymiaScores" + ext);
