...
public class QymiaPresetPanelController extends BaseController implements Initializable {

	// Every class need a logger...
	private static final Logger logger = LoggerFactory.getLogger(QymiaPresetPanelController.class);
...
//				update scriptComboBox
		...
		selectPresetDirMenuItem.setOnAction(e->{
			File dirBase = qupath.getProject() != null ? Projects.getBaseDirectory(qupath.getProject()) : new File(System.getProperty("user.home"));
			File presetDirFile = Dialogs.promptForDirectory("QymiaQuant Preset Directory", dirBase);
...
//			Dialogs.showErrorMessage("QymiaQuant Preset: Targets Invalid for ImageData", "Image does not contain all target channels/transforms in preset....");
			Dialogs.showWarningNotification("QymiaQuant Preset: Targets Invalid for ImageData", "Image does not contain all target channels/transforms in preset.... \nonly will work if quant image is mapped to this image via sync map.");
//			startQuantButton.setDisable(true);
//			runForProjectMenuItem.setDisable(true);
//			return;
		}
...
//			QymiaQuantBackend.TileOption tileOption;
//			try {
//				tileOption = Enum.valueOf(QymiaQuantBackend.TileOption.class, (String) presetParams.get("tileOption"));
//			} catch(IllegalArgumentException ex){
//				logger.error("tileOption in preset is invalid! Cannot load/run preset... {}", presetParams.get("tileOption"));
//				startQuantButton.setDisable(true);
//				cancelQuantButton.setDisable(true);
//				runForProjectMenuItem.setDisable(true);
//				Dialogs.showErrorMessage("QymiaQuant Preset: Tile Option Invalid", "Preset contains invalid tile option....");
//				ex.printStackTrace();
//				return;
//			}
//			startQuantButton.setDisable(false);
...
//					trying to find the batchMap file in the default folder if there is one....
			...
			if(!syncMapPath.isEmpty()){
				syncMap = QymiaAnalysisPanelController.loadTwoColMap(syncMapPath);
			}else{
						...
						if(convertMeasurementsProperty.get()){
							if(batchMap != null && !allMeasConvList.isEmpty()){
								...
								if (currentMeasConvs != null) {
									QymiaAnalysisPanelController.calculateMeasurementConversions(quantImageData, currentMeasConvs);
								} else{
...
//		set MenuItem params only
		...
		presetParams.put("tileOption", Enum.valueOf(QymiaQuantBackend.TileOption.class, (String) presetParams.get("tileOption")));
...
//			get controller and loadQuantPreset with current preset (file)
			QymiaQuantPanelController quantPanelController = (QymiaQuantPanelController) sceneManager.getController("/QymiaQuantPanel.fxml");
